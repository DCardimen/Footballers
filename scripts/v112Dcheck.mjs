...
ok(P1.pages.length === 5, 'the screen is five pages', P1.pages.map(p => p.id).join('/'))
...
ok(/STEP 1 OF 5/.test(P1.kick) && /INVOLVEMENT/i.test(P1.kick), 'page 1 announces itself', P1.kick + ' — "' + P1.title + '"')
...
ok(P1.dots.length === 5 && P1.dots[0] === 'on', 'a page indicator says where he is', P1.dots.join('|'))
...
ok(P5.page === 4 && P5.pages[4].shown && /STEP 5 OF 5/.test(P5.kick) && /WHEEL/i.test(P5.kick), 'page 5 is the wheel', P5.kick)
...
const F5 = await page.evaluate(() => { const imp = document.getElementById('v135ImpD'); const h = window.__V135.hold(); return { roll: !!document.getElementById('gv62roll'), standing: !!document.querySelector('#v112Page5 #growthV42'), result: !!document.querySelector('#v112Page5 #gv42out'), imp: imp ? imp.innerText.replace(/\s+/g, ' ') : null, applied: h && h.applied, next: document.getElementById('v112Next').innerText, dis: document.getElementById('v112Next').disabled, dupIds: (() => { const ids = [...document.querySelectorAll('#pregameV1513 [id]')].map(e => e.id); return ids.filter((id, i) => ids.indexOf(id) !== i) })(), buffs: (window.__V111_UI.player()._tempStatBuffsV25 || []).length, sheet: !!document.querySelector('#v135Final [data-id="preStatsV25"]') } })
ok(!F5.roll && F5.standing && F5.result, 'CONTINUE closes the roll pop-up and leaves the landed wheel and its result standing on the page', JSON.stringify({ roll: F5.roll, standing: F5.standing, result: F5.result }))
...
ok(F5.sheet && !F5.dupIds.length, 'the sheet is under it, carrying the swing, with no duplicated ids', 'dup=' + F5.dupIds.join(','))
ok(!F5.dis && /CONTINUE TO MATCH/i.test(F5.next), 'and now the button into the game is the primary one', F5.next)
await tapBack(); await tapNext()
const R5 = await page.evaluate(() => ({ standing: !!document.querySelector('#v112Page5 #growthV42'), spins: window.__V135.spins, buffs: (window.__V111_UI.player()._tempStatBuffsV25 || []).length, next: document.getElementById('v112Next').innerText }))
ok(R5.standing && R5.spins === 1 && R5.buffs === F5.buffs && /CONTINUE TO MATCH/i.test(R5.next), 'BACK and forward: the landed wheel is still standing, not respun, not re-applied', JSON.stringify(R5))
...
console.log('     shots', `${SHOT}_p1..p5.png`)
