...
| `wheelcheck.mjs` | v50 decision wheel, all three claims: the wheel's **wedge arcs are the personality weights** (same seed, two characters, the shares move and stay unequal); the **fit roll is its own system** (band odds total 100%, a no-opinion character lands a near-even three-way split, high jive is heavily positive, low jive heavily negative, and over 400 rolled careers the green rate tracks jive); and **tapping runs the rest at 5×** (rate reads 5 mid-roll, and the tapped roll finishes ~4× sooner than the untouched one). Also samples the live canvas ring to prove the wedges are cut to unequal arcs. Writes `_wheel.png` + `_wheel_landed.png`. Exits non-zero on any failure. |
