...
await page.addInitScript(() => { window.__P101 = []; let t0 = 0
  const desc = Object.getOwnPropertyDescriptor(Element.prototype, 'innerHTML')
  Object.defineProperty(Element.prototype, 'innerHTML', { configurable: true, get () { return desc.get.call(this) }, set (v) { try { if (this.querySelector && this.querySelector('.rib-liveload-v94')) window.__P101.push([performance.now() | 0, 'innerHTML over loader on #' + (this.id || this.className), new Error().stack.split('\n').slice(2, 7).join(' | ')]) } catch (e) {} return desc.set.call(this, v) } })
  const rc = Element.prototype.removeChild
  Element.prototype.removeChild = function (c) { try { if (c && c.classList && (c.classList.contains('rib-liveload-v94') || (c.querySelector && c.querySelector('.rib-liveload-v94')))) window.__P101.push([performance.now() | 0, 'removeChild', new Error().stack.split('\n').slice(2, 7).join(' | ')]) } catch (e) {} return rc.call(this, c) }
  const rw = Element.prototype.replaceWith
  setInterval(() => { const L = window.__LIVELOAD_V94, c = L && L.current; const el = document.querySelector('.rib-liveload-v94'); const st = (el ? 'el' : '-') + (c ? (c.done ? 'D' : 'L' + (c.field ? 'F' : '') + (c.ready ? 'R' : '')) : 'N'); if (st !== window.__st9) { window.__P101.push([performance.now() | 0, 'st', st]); window.__st9 = st } }, 20)
})
...
for (let i = 0; i < 60; i++) { await page.waitForTimeout(1000); if (await page.evaluate(() => window.__P101.filter(x => x[1] === 'st' && /-/.test(x[2])).length >= 1 && window.__P101.length > 3)) break }
