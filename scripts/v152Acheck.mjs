...
// ============================== 3. a real career end, restored at boot ==============================
{
  ...
  const tab = await E(() => { const t = document.querySelector('.hubv75-tab[data-sec="xp"]'); return t && { pulse: t.classList.contains('lg-tab-pulse'), pending: !!document.querySelector('.legacy-card-v152[data-pending]'), pours: window.__V152A_UI.pours.length } })
  ok(tab && tab.pulse && tab.pending && tab.pours === 0, 'the career end keeps the pour on its own LEGACY XP tab, pulsing, and waits for it to be opened', tab)
  await tap('.hubv75-tab[data-sec="xp"]')
  await page.waitForTimeout(1600); await shot('3_pour_mid')
  ...
  const d = await E(() => { const c = document.querySelector('.legacy-card-v152'); return { sum: (c.querySelector('.lgc-sum') || {}).textContent, num: +c.querySelector('.lgc-num').textContent, medal: +c.querySelector('.lgc-slot .lg-medal-v152:not(.lg-shard-v152)').dataset.rank, n: c.querySelectorAll('.lgc-slot .lg-medal-v152').length, swaps: window.__V152A_UI.swaps, pours: window.__V152A_UI.pours.length, seen: JSON.parse(localStorage.getItem('rib.legacy.v152') || '{}') } })
  ok(d.num === r.last.to && d.medal === Math.min(500, r.last.to) && d.n === 1 && d.pours === 1 && d.swaps >= 1 && d.seen.award === +r.award && d.sum === '+' + (r.last.to - r.last.from) + ' RANKS', 'the pour ran rank by rank to the new rank, swapping the medal on the way, stamped the total, and was marked seen', d)
...
// ============================== 5. milestones ==============================
{
  ...
  await page.waitForTimeout(2200); await shot('5_milestone')
  const o = await E(() => { const o = document.querySelector('.lgm-v152'); return o && { title: o.querySelector('.lgm-title').textContent, big: o.classList.contains('big'), medal: +(o.querySelector('.lgm-slot .lg-medal-v152:not(.lg-shard-v152)') || {}).dataset?.rank } })
  ...
  await page.waitForTimeout(2300); await shot('5_milestone_100')
...
// ============================== 7. the profile ==============================
{
  ...
  // a real drag: the page follows the finger and turns
  const sr = await E(() => { const r = document.querySelector('.lgb-stage').getBoundingClientRect(); return { x: r.left + r.width * 0.8, y: r.top + r.height * 0.5, w: r.width } })
  await page.mouse.move(sr.x, sr.y); await page.mouse.down(); for (let i = 1; i <= 8; i++) { await page.mouse.move(sr.x - i * sr.w * 0.08, sr.y); await page.waitForTimeout(16) }
