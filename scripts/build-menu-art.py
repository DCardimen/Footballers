...
# v106: each photograph's kit is segmented at pixel level by its own script — scripts/menu-kit-hero.py,
# scripts/menu-kit-card.py, scripts/menu-kit-portrait.py — which write the _p (jersey, primary colour)
# and _s (helmet + pants, secondary) alpha masks straight into public/menu/. They run here so one
# command still rebuilds everything; each also runs on its own and prints its own coverage/bleed report.
import runpy, sys
for kit in ['menu-kit-hero.py', 'menu-kit-card.py', 'menu-kit-portrait.py']:
