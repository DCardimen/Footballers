...
| 31 | `src/31-legacy.js` | 120K | `window.RIB_LEGACY` — the Legacy Rank: the 500 medals (the generated `RIB_LEGACY_MEDALS_V152` block, `scripts/build-legacy-medals.py`), the XP pour and the medal swap, the milestone / Rank 500 sequences, the rank chips, the profile's trophy case and collection book (v152 A, a new file after `30-music.js`; the ledger is in `07`) | `v152 A THE LEGACY RANK` |
| 30 | `src/30-music.js` | 30K | `window.RIB_MUSIC` — the music (the anthem streamed by two crossfading `<audio>` decks through WebAudio, the gesture, the pause, the duck) and `sfxOut(ctx)`, the effects bus the other sound paths opt into (v151 E, a new file after `29-seasons.js`, before the baked menu block — the coach and the vault opt into its effects bus) | `v151 E THE BAND PLAYS` |
