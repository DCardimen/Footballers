...
| 30 | `src/30-music.js` | 30K | `window.RIB_MUSIC` — the music (the anthem streamed by two crossfading `<audio>` decks through WebAudio, the gesture, the pause, the duck) and `sfxOut(ctx)`, the effects bus the other sound paths opt into (v151 E, a new file after `26-platform.js`) | `v151 E THE BAND PLAYS` |
