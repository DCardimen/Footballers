...
Every suite in `scripts/checks.json` (82 suites, 147 checks; `fast` and `full` are
...
|---|---|---|---|---|
| `smoke` | fast and broad — boot to menu, the shell, a career screen or five, the pregame, the live field, the engine; every check here is under ~3 min and passes on main. Run before every commit. | `namecheck`, `walk`, `shot`, `menu-integration-check`, `honorcheck`, `v85check`, `v146Echeck`, `v112Dcheck`, `v144check`, `simcheck`, `v110check`, `v146Bcheck`, `v147Ccheck`, `scrollcheck`, `freshcheck`, `v142check`, `agecheck`, `traincheck`, `layoutcheck`, `anchorcheck`, `v150Acheck`, `v151Echeck`, `v156Acheck` | — |  |
...
| `career-uff` | v147 A — retiring above college, sim the rest at 7+, stale offers, auto-answered story decisions | `v156Bcheck`, `v154Acheck`, `v147Acheck`, `v146Bcheck`, `rivalcheck`, `v90check`, `v85check`, `v136check`, `declarecheck`, `v88check`, `bootviewcheck`, `v146Echeck`, `walk` | `v147 A`, `v154 A` |  |
| `menu-goal` | v153 D — the menu's goal trophy (UFF → Interstellar), the Legacy medal box under the logo, the PROFILE tile, the prestige coin tile, the breathing players, 400 wide | `v153Dcheck`, `v147Bcheck`, `v152Acheck`, `menu-integration-check`, `heroflashcheck`, `menufxcheck` | `v153 D`, `scripts/build-goal-trophies.py`, `art/menu/trophy_*`, `public/menu/trophy_goal_*` | first: `python3 scripts/build-goal-trophies.py` (if the trophy art changed), then `python3 scripts/art-provenance.py`; `RIB_MENU_VERSION=<stamp> node scripts/bake-menu-into-index.mjs` (any `public/rib-menu*` file) · look: `SHOTS=<dir> node scripts/v153Dcheck.mjs` |
| `menu-coin` | v147 B — the menu prestige coin, the OVR ring arc and spark, the milestones trophy | `v147Bcheck`, `menu-integration-check`, `menufxcheck`, `heroflashcheck`, `sheencheck`, `honorcheck`, `freshcheck`, `menu-mask-check`, `v156Acheck` | `v147 B`, `scripts/build-uff-trophy.mjs`, `public/menu/**` | first: `node scripts/build-uff-trophy.mjs` (if the trophy art changed); `RIB_MENU_VERSION=<stamp> node scripts/bake-menu-into-index.mjs` (any `public/rib-menu*` / `rib-vault*` file) · look: `CAREER=1 node scripts/menushot.mjs` |
...
| `contact` | v146 A — who is on him when he goes down, the fall together, the taken sack | `v146Acheck`, `v143check`, `scoreneutralcheck`, `tacklecheck`, `jukecheck`, `creditcheck`, `v103check`, `v109C1check`, `v112Fcheck`, `renderpathcheck`, `v86check`, `v110check`, `v151Dcheck`, `v153Acheck` | `v146 A`, `v151 D`, `v153 A` |  |
| `cuts` | v146 B — UFF cuts, the strike count (v154 A: per career, 1 by default, Free Agency buys a 2nd), the three-offer club screen | `v154Acheck`, `v146Bcheck`, `declarecheck`, `v88check`, `bootviewcheck`, `namecheck`, `v111Bcheck`, `v85check`, `menu-integration-check`, `honorcheck`, `walk` | `v146 B`, `v154 A` |  |
| `impossible` | v146 C — the Impossible branch, the over-cap price, the wall, the Evergreen refund | `v146Ccheck`, `agecheck`, `capcheck`, `vaultcheck`, `honorcheck`, `v134check`, `v136check`, `bootviewcheck`, `walk`, `v156Acheck` | `v146 C` |  |
...
| `vault` | v137 — the Prestige Vault: hoard, spend, door, payout, sprites; v153 C — the payday (the reward sequence) | `vaultcheck`, `v153Ccheck`, `honorcheck`, `v134check`, `coachcheck`, `menu-integration-check`, `v136check`, `freshcheck`, `v156Acheck` | `v137`, `v153 C`, `public/rib-vault*`, `public/vault/**`, `scripts/build-vault-art.py`, `scripts/vaultcut.py` | first: `python3 scripts/build-vault-art.py --proof` (if a cell moved — then LOOK at `art/vault-proof/`); `RIB_MENU_VERSION=<stamp> node scripts/bake-menu-into-index.mjs` (any `public/rib-menu*` / `rib-vault*` file) · look: `vaultshot.mjs` / `WIDE=1 vaultshot.mjs` / `vaultspend.mjs` / `vaultdoor.mjs` / `vaultphys.mjs`; `v153Cshot.mjs` (the payday; `GAINS=12,1284,250000`, `RECORD=1`, `REDUCED=1`) |
...
| `goals` | v134 — goal payouts, Hall of Fame box scores, Apex branch, ring soft max, season strip | `v134check`, `coachcheck`, `growcheck`, `menufxcheck`, `menu-integration-check`, `honorcheck`, `stridecheck`, `rivalcheck`, `v85check`, `declarecheck`, `walk`, `v156Acheck` | `v134` |  |
...
| `snaps` | v111 / v120 — the pregame ladder's snaps, coach trust, the fatigue slope | `v111Acheck`, `v111Bcheck`, `v111Ccheck`, `v112Dcheck`, `v85check`, `creditcheck`, `simcheck`, `scoreneutralcheck`, `faqcheck`, `v153Acheck` | `v111`, `v120` |  |
...
| `pile` | v103 / v109 C1 — the grab, the pile, the strip, impact geometry, the bobble | `v109C1check`, `v103check`, `readcheck`, `tacklecheck`, `creditcheck`, `simcheck`, `renderpathcheck`, `v86check`, `v153Acheck` | `v103`, `v109 THE HIT` |  |
...
| `honors` | v130 / v156 A — the account rank (the medals since v156 A), the prestige-tree gates, anything drawing a ★ | `honorcheck`, `menu-integration-check`, `walk`, `shot`, `v153Fcheck`, `v156Acheck` | `v130`, `v156 A`, `MEDALS ARE THE KEY` |  |
...
| `monetize` | v149 E / v150 C / v151 A — the monetization module, its in-game hooks (switched off by default) and the v151 A progression gates | `v156Bcheck`, `v149Echeck`, `v150Ccheck`, `v151Acheck`, `bootviewcheck`, `walk`, `menu-integration-check`, `layoutcheck`, `honorcheck`, `vaultcheck`, `v156Ccheck` | `v149 E`, `v150 C`, `v151 A`, `RIB_MONETIZE`, `v156 C`, `src/27-monetize.js` |  |
...
| `seasons` | v151 C — competitive seasons, the Career Pass, the career leaderboards, the trophy case (src/29-seasons.js, src/20-leaderboards.js v151 C) | `v156Ccheck`, `v151Ccheck`, `v153Gcheck`, `lbcheck`, `dailycheck`, `bootviewcheck`, `menu-integration-check`, `v146Echeck`, `v150Ccheck`, `layoutcheck`, `walk` | `v151 C`, `RIB_SEASONS`, `v156 C`, `src/29-seasons.js`, `src/20-leaderboards.js`, `docs/SEASONS.md` |  |
| `cosmetics` | v151 B — cosmetics and the profile: RIB_COSMETICS, the kit / helmet / celebration / stadium / vault hooks, the Locker's STYLE tab, the PROFILE view, the Team Creator's gated crests and colours (src/28-cosmetics.js) | `v157Acheck`, `v156Ccheck`, `v151Bcheck`, `v153Gcheck`, `v151Ccheck`, `kitsidecheck`, `emblemcheck`, `vaultcheck`, `bootviewcheck`, `menu-integration-check`, `v146Echeck`, `layoutcheck`, `walk`, `v157Bcheck`, `v157Ccheck` | `v151 B`, `RIB_COSMETICS`, `v153 G`, `v156 C`, `v157 A`, `v157 B`, `v157 C`, `src/28-cosmetics.js`, `docs/COSMETICS.md` |  |
...
| `legacy` | v152 A — the Legacy Rank: the 500 medals (src/31-legacy.js, the RIB_LEGACY_MEDALS_V152 block, scripts/build-legacy-medals.py), the ledger in 07 (legacy*V152), the pour and the medal swap, milestones, Rank 500, the profile's trophy case and collection book, the rank chips | `v157Ccheck`, `v152Acheck`, `v151Ccheck`, `v151Bcheck`, `bootviewcheck`, `layoutcheck`, `menu-integration-check`, `v153Fcheck`, `v153Echeck`, `v156Acheck` | `v152 A`, `RIB_LEGACY`, `v153 F`, `v157 C`, `src/31-legacy.js`, `public/legacy/**`, `scripts/build-legacy-medals.py`, `art/legacy/**` |  |
...
| `thumb` | v153 E — phone fit: the profile figure's kit recolour (src/28-cosmetics.js drawCharacter), the one-page skill sheet (07 screenUpgrade), the 11px / 36px floors on the career screens (25-shell THE THUMB, 22's stats tabs), locked Legacy medals black (31) | `v153Echeck`, `v146Echeck`, `scrollcheck`, `traincheck`, `capcheck`, `v142check`, `v151Bcheck`, `v152Acheck`, `v150Acheck`, `layoutcheck` | `v153 E` |  |
| `v156b` | v156 B — season sims earned with the Legacy medal groups (a career's, 1 → 20; the ad 30 min unlimited) and the playoffs always played live (07's v156 B block, simSeasonV147, postV147, screenSeason's dock, the live SKIP; 27's simUnlimited) | `v156Bcheck`, `v151Acheck`, `v147Acheck`, `v150Ccheck`, `v149Echeck`, `v85check`, `v90check`, `injurycheck`, `coachcheck`, `growcheck`, `v146Echeck`, `bootviewcheck`, `walk` | `v156 B`, `scripts/v156Bcheck.mjs` |  |
| `v156d` | v156 D — MY PLAYS ONLY on the live field (the ticked box, the playoffs' every-snap-on-your-side rule) | `v156Dcheck`, `v153Bcheck`, `v151Acheck`, `coachcheck` | `v156 D` |  |
