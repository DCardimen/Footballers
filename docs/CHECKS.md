...
Every suite in `scripts/checks.json` (79 suites, 136 checks; `fast` and `full` are
...
| `legacy` | v152 A — the Legacy Rank: the 500 medals (src/31-legacy.js, the RIB_LEGACY_MEDALS_V152 block, scripts/build-legacy-medals.py), the ledger in 07 (legacy*V152), the pour and the medal swap, milestones, Rank 500, the profile's trophy case and collection book, the rank chips | `v152Acheck`, `v151Ccheck`, `v151Bcheck`, `bootviewcheck`, `layoutcheck`, `menu-integration-check`, `v153Fcheck` | `v152 A`, `RIB_LEGACY`, `v153 F`, `src/31-legacy.js`, `public/legacy/**`, `scripts/build-legacy-medals.py`, `art/legacy/**` |  |
