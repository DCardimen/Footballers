...
- `window.__FieldSim` — the public API: `.pass(...)` / `.run(...)` resolve a play
  ...
  popping the head — see the render-path fix notes in `docs/CHANGELOG.md`).

## The emergent game engine `simGameV2` (`Yr`) — `/* ===== v16 EMERGENT GAME ENGINE`

`window.__simGameV2(perf, pos)` = `simGameV2` (`Yr` before v149 C). Resolves a full game play-by-play on a
real clock and returns `{plays, usScore, themScore, stat, team, oppTeam, roster}`.
Inside it:

- `buildGameRosters(...)` (`Wr`) builds both rosters; the you-player is injected at his position.
...
The checks run by suite through `scripts/run-checks.mjs` (`docs/CHECKS.md`); it starts its own dev servers.

```bash
node scripts/run-checks.mjs --since main --jobs 4   # the suites your diff touches
node scripts/run-checks.mjs smoke --jobs 4          # before every commit
node scripts/run-checks.mjs credit sim reads        # the engine's invariants: stat credit, distributions, reads
node scripts/run-checks.mjs --list                  # every suite and what it covers
```

The ones this document leans on, by hand (`npm run dev` first, or set `GAME_URL`):
...
node scripts/scoreneutralcheck.mjs # anything in the sim's feel: keep the before row, compare several seeds
node scripts/blowoutcheck.mjs      # v76 margin curve, both score paths (slow; BLOW_N=400)
node scripts/teamqualcheck.mjs     # v68 prestige-tree nerf holds at ~10x
node scripts/equaltalentcheck.mjs  # mirrored rosters are actually fair (loads the engine and 07 by file, no server)
...
See `scripts/README.md` for the full catalog (including screenshot/exploration helpers). Chromium is
pre-installed at `/opt/pw-browsers/chromium` and wired through `scripts/lib/env.mjs`.
...
- **Match the local density.** The career app (`src/07-career-app.js`) is formatted
  with real names since v149 C — keep it that way; FieldSim is written spaciously
  with banner comments. Match whichever region you're editing.
- **Don't touch the Phaser bundle** (`src/vendor/phaser.min.js`). Sheets are files
  in `public/`, loaded through `window.__RIB_ASSET` — no new data URLs.
- **Version-stamp new systems** in a banner comment (`vNN …`), add the anchor to
  `docs/ANCHORS.md`, and summarize the change at the top of `docs/CHANGELOG.md` —
  that log is the project history.
