...
- **v81 REAL FOOTBALL** (anchor `v81 REAL FOOTBALL`): the run game blocks eleven, not four. WR stalk blocks (`_stalk`/`_stalkBy`, engaged via `stalkTick`, fought and shed in the pursuit loop with urgency once the ball is past), TE seals a LINEBACKER (never a safety — sealing the roof is how 80-yard runs happen), uncovered OL climb to the nearest live backer, and the run double-team is deleted (`kind==="pass"` only). One safety roofs (the deeper), the other FILLS; contain belongs to the corners. `DEF_LX` backers align at ~5 yards (`alignLB`), not nine. Pursuit deals ROLES per tick (`pursuitRole`): two attack, two leverage a spot ahead (`levGap`/`levLeadMax`), the rest rally (`rallyGap`/`rallyPace`) — and attacks at `pursuitEffort` (1.15) because the carrier surges at up to 1.42× through the hole (`holeSurge` + sprint) and parity is what ends the track meet. Contact finishes: `swarmChoke` 1−0.42^hands, `wounded` (a recent stagger halves every evasion for ~1.1s), `_isLast` discipline (the last man is far harder to whiff/truck/hurdle), `truckMomCap` (momentum aids a truck, it isn't one), and `endTackle` converts the pile's `_drive` into REAL yards (`fallFwdK`) so the dead-ball spot matches the rendered churn. The lane read (`laneScanLx`/`laneHatY`) scores holes by maintained blocks MINUS free hats in the corridor. v81.1 TALENT RESPONSE: the open-field footrace is GRADED (`trailFadeK2`/`trailFadeMax` — a trailing defender's pace fades with the square of his speed deficit) instead of the old spd+6 trail-pace / runDown-margin cliffs, which flipped every defender on the worse team at once and made a +4 team attribute gap triple run YPC (in-play advantages compound: line holds longer × wider hole × more contacts won × footrace fades less). Pass plays add `yacDrive` to pursuit (the ball is out, everyone drives on the catch). `talentParity` (in `makeAgents`) can compress the between-team mean attribute gap, each player keeping his deviation from his own team — default 1 = raw gap; lower it only alongside a blowoutcheck retune. The v76 stand-down cap `standMax` went .34 → .28: it, not `standGapK`, was the binding term in the +6..+10 OVR bands after v81 moved the run game. All dials TU()-tuned; guarded by `scripts/trenchcheck.mjs` (pure Node) with movementcheck's ±12% calibration corridor as the yardage gate and blowoutcheck's margin curve as the talent-response gate.
